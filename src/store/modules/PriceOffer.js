...
const actions = {
    ...
    async show({ commit }, one) {
        ...
        let path = `/get_price_offer_details/${one}`;
        const response = await this.$axios.get(path);
    ...
    async store({ commit }, data) {
        try {
            ...
            let path = `/add_price_offer/`;
            const response = await this.$axios.post(path, data);
            this.dispatch("setAlert", {
                type: "success",
                text: "تمت العملية بنجاح "
            });
            this.dispatch("setRedirect","price_offer.index");
        } catch (error) {
            this.dispatch("setAlert", {
                type: "error",
    ...
    async delete({ commit }, data) {
        try {
            ...
            const response = await this.$axios.delete(`/delete_price_offer/${data}`);
            this.dispatch("setAlert", {
                type: "success",
                text: "تمت العملية بنجاح "
            });
    ...
    async confirmPriceOffer({ commit }, data) {
        let response = await this.$axios.post(`/admin_confirm_price_offer/${data.PK}`);
        this.dispatch("setAlert",{
            type:"success",
            text:"تمت العملية بنجاح "
        });
