    ...
    <v-row justify="center" dense>
      <v-col cols="12">
        <v-card outlined>
          <v-card-title>
            <v-row>
              <v-col cols="10"> الموظفون </v-col>
              <v-col cols="2">
                <v-btn to="/employee/form" dark color="primary">
                  إضافة موظف جديد
                </v-btn>
              </v-col>
            </v-row>
          ...
          <v-card-text>
            <v-data-table :headers="headers" :items="all" :items-per-page="5">
              <template v-slot:item.actions="{ item }">
                <v-btn icon @click="add_task(item)" title="إضافة مهمة">
...
export default {
  ...
  mounted() {
    if (this.$store.state.auth.user.role == "Admin")
      ...
      this.$store.dispatch("employee/get_design_employees");
  },
  computed: {
    ...mapState({
